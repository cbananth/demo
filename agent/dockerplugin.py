        ...
        for key, values in blkio_stats.items():
            ...
            for type_instance, values in device_stats.items():
                if len(values) == 5:
                    ...
                    excollectd.warn(('Unexpected number of blkio stats for '
                                   'container {container}!')
...
class ContainerStats(threading.Thread):
    ...
    def run(self):
        excollectd.info('Starting stats gathering for {container}.'
        ...
        while not self.stop:
            try:
                ...
                excollectd.warning('Error reading stats from {container}: {msg}'
        ...
        excollectd.info('Stopped stats gathering for {container}.'
                      .format(container=_c(self._container)))
...
class DockerPlugin:
    ...
    def __init__(self, id, docker_url=None):
    ...
    def init_callback(self):
        ...
        try:
            ...
            excollectd.warning(('Docker daemon at {url} does not '
                              'support container statistics!')
                             .format(url=self.docker_url))
            return False

        excollectd.register_read(self.read_callback)
        excollectd.info(('Collecting stats about Docker containers from {url} '
...
#             del self.stats[cid]
        ...
        stats = ContainerStats(self.id,self.client)
        return stats
...
class PluginClass:
    ...
    def __init__(self, id):
            excollectd = self.ExecCollectd()
            plugin = DockerPlugin(id)
            if len(sys.argv) > 1:
    ...
    class ExecCollectd:
        ...
        def Values(self):
            return PluginClass.ExecCollectdValues(self.id)
